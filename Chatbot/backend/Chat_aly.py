...
# FILE PATHS
# -------------------------------
RAW_TRANSCRIPT_FILE = "chatbot_debate_transcript.txt"
...
def detect_argument_type(sentence):
    ...
    if any(k in s for k in ["i think", "i believe"]):
        return "Claim"

    if any(k in s for k in ["because", "since", "due to"]):
        return "Evidence"

    if any(k in s for k in ["but", "however", "although"]):
        return "Rebuttal"
